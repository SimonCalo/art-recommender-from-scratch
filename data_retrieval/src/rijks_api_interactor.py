        ...
        while n_images_to_download > 0:
            ...
            for item in page_response["orderedItems"]:
                ...
                if artwork_dict == {}:
                    ...
                    continue
                visual_item_dict = self.get_response_dict(
                    url=artwork_dict["shows"][0]["id"]
                )
                if visual_item_dict == {}:
                    continue
                if not self.passes_requirements(visual_item_dict, is_nested=True):
                    continue
                digitally_shown_id = visual_item_dict["digitally_shown_by"][0]["id"]
